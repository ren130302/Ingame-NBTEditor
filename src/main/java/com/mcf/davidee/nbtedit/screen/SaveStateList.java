...
public class SaveStateList extends ObjectSelectionList<SaveStateEntry> {
	...
	public OnPress saveButton() {
		return button -> {
			...
			com.put(tag.name(), tag.tag());
			this.addEntry(new SaveStateEntry(tag.name(), com));
			this.save();
			this.load();
		};
	}
	...
	public OnPress loadButton() {
		return button -> {
			SaveStateEntry save = this.getSelected();
			NBTTree.clipboard = NamedNBT.of(save.tag);
		};
	}

	private final String key_slots = "slots";
	...
	public void save() {
		try {
			...
			for (SaveStateEntry entry : this.children()) {
				...
				slot.put(this.key_tag, entry.tag);
				slot.putString(this.key_label, entry.name);
				System.out.println("name:" + entry.name);
				slots.add(slot);
			}
			root.put(this.key_slots, slots);
	...
	public void load() {
		...
		try {
			if (NBTEdit.NBT_DAT.exists() && NBTEdit.NBT_DAT.canRead()) {
				...
				ListTag slots = (ListTag) root.get(this.key_slots);
				if (Objects.nonNull(slots) && !slots.isEmpty()) {
					for (int i = 0; i < slots.size(); ++i) {
						CompoundTag slot = (CompoundTag) slots.get(i);
						CompoundTag tag = slot.getCompound(this.key_tag);
						String label = slot.getString(this.key_label);
						System.out.println("name:" + label);
						this.addEntry(new SaveStateEntry(label, tag));
					}
				}
			}
	...
	public class SaveStateEntry extends ObjectSelectionList.Entry<SaveStateEntry> {
		...
		public SaveStateEntry(String name, CompoundTag tag) {
			this.name = name;
			this.tag = tag;
		...
		private void setEdited(boolean isEditable, boolean needSave, boolean needLoad) {
			...
			if (!this.isEditable && needSave) {
				this.name = this.editBox.getValue();
				SaveStateList.this.save();
			}
			if (needLoad) {
