...
@OnlyIn(Dist.CLIENT)
/**
 * Copy EditBox modify insertText (filterText)
 *
 * @author ren130302
 */
public class NEditBox extends AbstractWidget implements Widget, GuiEventListener {
	...
	public NEditBox(Font p_94114_, int p_94115_, int p_94116_, int p_94117_, int p_94118_, Component p_94119_) {
		this(p_94114_, p_94115_, p_94116_, p_94117_, p_94118_, (NEditBox) null, p_94119_);
	}

	public NEditBox(Font p_94106_, int p_94107_, int p_94108_, int p_94109_, int p_94110_, @Nullable NEditBox p_94111_,
			Component p_94112_) {
	...
	public boolean mouseClicked(double p_94125_, double p_94126_, int p_94127_) {
		if (!this.isVisible()) {
			...
			boolean flag = p_94125_ >= this.x && p_94125_ < this.x + this.width && p_94126_ >= this.y
					&& p_94126_ < this.y + this.height;
	...
	public void renderButton(PoseStack p_94160_, int p_94161_, int p_94162_, float p_94163_) {
		if (this.isVisible()) {
			...
			if (!s.isEmpty()) {
				...
				j1 = this.font.drawShadow(p_94160_, this.formatter.apply(s1, this.displayPos), l, i1, i2);
			}
			...
			if (!s.isEmpty() && flag && j < s.length()) {
				this.font.drawShadow(p_94160_, this.formatter.apply(s.substring(j), this.cursorPos), j1, i1, i2);
			}

			if (!flag2 && this.suggestion != null) {
				this.font.drawShadow(p_94160_, this.suggestion, k1 - 1, i1, -8355712);
			}
			...
			if (flag1) {
				if (flag2) {
					...
					this.font.drawShadow(p_94160_, "_", k1, i1, i2);
				}
			}
	...
	public boolean isMouseOver(double p_94157_, double p_94158_) {
		return this.visible && p_94157_ >= this.x && p_94157_ < this.x + this.width && p_94158_ >= this.y
				&& p_94158_ < this.y + this.height;
	}
	...
	public static String filterText(String str) {
		...
		for (char c : str.toCharArray()) {
			if (SharedConstants.isAllowedChatCharacter(c) || (c == NBTStringHelper.SECTION_SIGN || c == '\n')) {
				sb.append(c);
			}
		}
		return sb.toString();
